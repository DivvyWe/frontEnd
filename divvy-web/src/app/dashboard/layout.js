...
export default async function DashboardLayout({ children }) {
  ...
  console.log("[dash/layout] hasToken:", !!token);
  if (!token) {
    console.log("[dash/layout] redirect -> /auth/signin (no token)");
  ...
  console.log("[dash/layout] /auth/me status:", meRes.status);
  if (!meRes.ok) {
    const raw = await meRes.text();
    console.log("[dash/layout] /auth/me body:", raw.slice(0, 300));
    console.log("[dash/layout] redirect -> /auth/signin (meRes not ok)");
    redirect("/auth/signin");
  }

  const me = await meRes.json();
  console.log("[dash/layout] me:", me);
